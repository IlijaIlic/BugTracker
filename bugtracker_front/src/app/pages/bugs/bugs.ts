...
export class Bugs implements OnInit {
  ...
  handleSubmitBug() {
    ...
    if (this.newBug!.description)
      formData.append('Description', this.newBug!.description);
  ...
  handleUpdate() {
    ...
    if (this.selBug!.platform)
      formData.append('Platform', this.selBug!.platform);
    if (this.selBug!.severity)
      formData.append('Severity', this.selBug!.severity);
    if (this.selBug!.priority)
      formData.append('Priority', this.selBug!.priority);
    if (this.selBug!.description)
      formData.append('Description', this.selBug!.description);
  ...
  bugMoreDetails(bug: BugModel) {
    ...
    this.selBug = { ...bug };
    if (bug.dateFixed)
      this.selBugDate = new Date(bug.dateFixed).toISOString().split('T')[0];
